/**
 * Based on the mock API for Apkallu Falls by James Donnelly
 * -- https://github.com/ApkalluFalls/api.apkallufalls.com
 **/
...
function process(data) {
  ...
  if (this.list) {
    ...
    return this.callback(data, this.dest, createJSON.bind(this))
  }

  const processData = (d) => {
    const fileName = d.id
    const filePath = `${this.dest}${fileName}.json`
      ...
      if (exists)
        return ((d, fileName, logMessage) => {
          fs.readFile(filePath, 'utf8', (e, data) => {
            // Do nothing if there are not changes
...
function createJSON(fileName, data, logMessage, isNew) {
  ...
  fs.writeFile(`${this.dest}${fileName}.json`, JSON.stringify(data), 'utf8', () => {
